---
title: "20/80 Bucket System"
description: "A pre-task triage rule that puts roughly 20% of your work in a 'deserves 110%' bucket and the other 80% in a 'good enough' bucket, before you start. A specific counter-move to perfectionism applied to the wrong tasks."
type: framework
dimension: Mindset
dimension_type: foundation
presenter: "Katarina Posa"
agency: "IOM"
session: "Managing Your Saboteurs"
session_slug: "cw-day-3-session-5"
...
1. **List the tasks.** Everything you are going to spend effort on: documents to write, applications to submit, conversations to prepare for, slides to design.
2. **Triage into two buckets.** For each task, decide before starting: this goes in the 20% bucket (110% effort, polished, multi-pass) or the 80% bucket (good enough, single pass, ship it).
3. **Set a stop rule for the 80% bucket.** Define what "good enough" means concretely. *"One draft, one read-through, send."* Or *"Version 1.0 only."* Without a stop rule, the Stickler pulls 80%-bucket tasks back across the line.
4. **Protect the 20% bucket.** That is where your best energy goes. The point is not that you do less work; it is that you do better work where it matters and stop wasting peak effort where it does not.
5. **Re-triage if a task changes context.** A task that started in the 80% bucket can move to the 20% bucket if it becomes load-bearing. The reverse is also true.
